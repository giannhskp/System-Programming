...
void bloomExists(bloomFilter bloomHead,char *id){ // check if an id has been inserted to the bloom filter
  ...
  if(flag){ // if one all the bits were set to 1, then the record may have been inserted
    printf("MAYBE\n");
  }else{  // if one of the bits was not set, then the record has not been inserted
