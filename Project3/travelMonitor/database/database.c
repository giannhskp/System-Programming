...
void readCitizenRecordsFile(Data database,char *fileName,pthread_mutex_t *updateDataMutex){
  pthread_mutex_lock(updateDataMutex);
  FILE *fp=fopen(fileName,"r");
  pthread_mutex_unlock(updateDataMutex);
  if(fp==NULL){
    perror("Error");
    return;
  }
  while(!feof(fp)){ // for every line of the file (every record)
    ...
    if(strcmp(vaccinated,"NO")==0 && read==8){
      ...
      if(strcmp(vaccinated,"NO")==0){ // correct NO record, so insert it
        ...
        pthread_mutex_lock(updateDataMutex);	// wait if another thread is updating the database
        if(validCountryOrName(firstName) && validCountryOrName(lastName) && validCountryOrName(country) && validAge(age) && validVirus(virusName)){
          // mutex is used when inserting a record to the database as it is a critical section
            insertFromFile(database,citizenId,firstName,lastName,country,age,virusName,vaccinated,NULL);
        }
        else{
          printf("ERROR IN RECORD %s\n",buffer);
        }
        pthread_mutex_unlock(updateDataMutex);	// release the mutex so as other thread can update the database
