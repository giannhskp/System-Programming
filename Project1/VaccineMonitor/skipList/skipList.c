...
countryCounterList populationStatusSL(countryCounterList list,skipList head,char *country,char *date1,char *date2,int vaccSkipList){
  slLevelNode *levelsList=head->topLevel;
  while(levelsList!=NULL){  // go to the bottom level of the skipList
    if(levelsList->level==1){
      break;
    }else{
      levelsList=levelsList->downLevel;
  ...
  if(levelsList!=NULL){
    slNode *l=levelsList->list;
    while(l!=NULL){ // for evey key in the bottom level of the skipList
      if(country==NULL){  // if the country was not given
        // we consider every citizen in the skipList (and categorize the by country inside the countryCounterList)
        citizenRecord rec=(citizenRecord)l->down; // take the record using the pointer to the record that is stored in bottom level nodes
        if(vaccSkipList){ // if we are on vaccinated_persons skipList
          if(date1!=NULL && date2 !=NULL){ //if dates were given
            if(insideDates(date1,date2,l->date)){   //if person was vaccinated inside the given dates
              // increase both the total counter and meetsConditions counter of this country
              list=increaseCounter(list,getCountry(rec),1);
            }else{    //if person was NOT vaccinated inside the dates
                // only increase the total counter and NOT meetsConditions counter of this country
                // (as the citizen was not vaccinated inside the given dates)
              list=increaseCounter(list,getCountry(rec),0);
            }
          }else{ //if dates were NOT given
            // increase both the total counter and meetsConditions counter of this country (as the citizen is vaccinated)
            list=increaseCounter(list,getCountry(rec),1);
          }
        }else{  // if we are on not_vaccinated_persons skipList
          // only increase the total counter and NOT meetsConditions counter of this country (as the citizen is in not_vaccinated_persons skipList)
          list=increaseCounter(list,getCountry(rec),0);
        }
      }else{  // if the country was given
        // we only consider citizen from the given country in the skipList
        // so the countryCounterList only has one node for the given country (as we only insert citizens from this country)
        citizenRecord rec=(citizenRecord)l->down;
        if(strcmp(country,getCountry(rec))==0){ // if the citizen's country is the same as the given one
          if(vaccSkipList){ // if we are on vaccinated_persons skipList
            if(date1!=NULL && date2 !=NULL){  // if dates were given
              if(insideDates(date1,date2,l->date)){ //if person was vaccinated inside the given dates
                // increase both the total counter and meetsConditions counter of this country
              ...
              // increase both the total counter and meetsConditions counter of this country (as the citizen is vaccinated)
              // and "we don't care" about the date that he was vaccinated
              list=increaseCounter(list,getCountry(rec),1);
            }
          }else{  // if we are on not_vaccinated_persons skipList
              // only increase the total counter and NOT meetsConditions counter of this country
              // (as the citizen is in not_vaccinated_persons skipList)
            list=increaseCounter(list,getCountry(rec),0);
          }
        }
      }
      // go to the next citizen
      l=l->next;
    }
  }
  return list;
...
countryAgeList popStatusByAgeSL(countryAgeList list,skipList head,char *country,char *date1,char *date2,int vaccSkipList){
  slLevelNode *levelsList=head->topLevel;
  while(levelsList!=NULL){  // go to the bottom level of the skipList
    if(levelsList->level==1){
      break;
  ...
  if(levelsList!=NULL){
    ...
    while(l!=NULL){ // for evey key in the bottom level of the skipList
      if(country==NULL){  // if the country was not given
        ...
        if(vaccSkipList){ // if we are on vaccinated_persons skipList
          if(date1!=NULL && date2 !=NULL){ //if dates were given
            if(insideDates(date1,date2,l->date)){   //if person was vaccinated inside the dates
              // increase both the total counter and age counter (that corresponds to the record's age) of this country
              list=increaseAgeCounter(list,getCountry(rec),getAge(rec),1);
            }else{            //if person was NOT vaccinated inside the dates
              // only increase the total counter and NOT age counter of this country
              // (as the citizen was not vaccinated inside the given dates)
              list=increaseAgeCounter(list,getCountry(rec),getAge(rec),0);
            }
          }else{ //if dates were NOT given
            // increase both the total counter and age counter (that corresponds to the record's age) of this country
            // because the citizen is vaccinated
            list=increaseAgeCounter(list,getCountry(rec),getAge(rec),1);
          }
        }else{  // if we are on not_vaccinated_persons skipList
          // only increase the total counter and NOT age counter of this country (as the citizen is in not_vaccinated_persons skipList)
          list=increaseAgeCounter(list,getCountry(rec),getAge(rec),0);
        }
      }else{
        // if the country was given
        // we only consider citizen from the given country in the skipList
        // so the countryCounterList only has one node for the given country (as we only insert citizens from this country)
        citizenRecord rec=(citizenRecord)l->down;
        if(strcmp(country,getCountry(rec))==0){ // if the citizen's country is the same as the given one
          if(vaccSkipList){ // if we are on vaccinated_persons skipList
            if(date1!=NULL && date2 !=NULL){  // if dates were given
              if(insideDates(date1,date2,l->date)){ //if person was vaccinated inside the given dates
                // increase both the total counter and corresponding age counter of this country
                list=increaseAgeCounter(list,getCountry(rec),getAge(rec),1);
              }else{  //if person was NOT vaccinated inside the dates
                  // only increase the total counter and NOT age counter of this country
                  // (as the citizen was not vaccinated inside the given dates)
              ...
              // increase both the total counter and age counter of this country (as the citizen is vaccinated)
              // and "we don't care" about the date that he was vaccinated
              list=increaseAgeCounter(list,getCountry(rec),getAge(rec),1);
            }
          }else{  // if we are on not_vaccinated_persons skipList
      ...
      l=l->next;  // go to the next citizen
    }
  }
  return list;
}
