...
int travelRequest(char citizenId[],char date[],char countryFrom[],char countryTo[],char virusName[], int fdboard[][2],bloomList bloomFiltersOfMonitors[], stringList countriesOfMonitors[]){
  ...
  if(exists){ // if it exist "ask" the monitor if the citizen is vaccinated
    ...
    sendMessage(fdboard[monitor][WRITE],tempBuff); // send the /travelRequest command to the monitor
    sendMessage(fdboard[monitor][WRITE],citizenId);  // send the citizenId to the monitor
    sendMessage(fdboard[monitor][WRITE],virusName);  // send the virusName to the monitor
    char *response=readMessage(fdboard[monitor][READ]);  // read the response from the monitor
    if(strcmp(response,"YES")==0){  // if the monitor responded YES
      ...
      char *vaccinationDate=readMessage(fdboard[monitor][READ]); // read the vaccination date of the citizen
      ...
      if(isInside6Months==1){  // check if the vaccination date is inside 6 moths from the travel date
        ...
        sendMessage(fdboard[monitor][WRITE],tempBuff2);  // inform the monitor that the request was accepted
        return 1;
        ...
        sendMessage(fdboard[monitor][WRITE],tempBuff2);  // inform the monitor that the request was rejected
        return 0;
      }
...
void addVaccinationRecords(char country[],int pidArray[],int fdboard[][2],bloomList (*bloomFiltersOfMonitors)[], stringList countriesOfMonitors[]){
  int monitor=findMonitorByCountry(country,countriesOfMonitors); // find to which monitor the given country is assigned
  if(monitor==-1){ printf("Country does not exist in the travelMonitor\n"); return; }
  kill(pidArray[monitor],SIGUSR1); // send a USR1 signal to this monitor in order to inform it that a new file was placed in a directory
  int bloomFilterSent=0;
  while(1){ // start reading the updated bloomFilters of the monitor
    char *virusName=readMessage(fdboard[monitor][READ]);
    if(strcmp(virusName,"ENDOFBLOOMFILTERS")==0){ // that means that the monitor sent all the bloomFilter
      ...
      bloomFilter bf = readBloom(fdboard[monitor][READ]);
      (*bloomFiltersOfMonitors)[monitor] = insertBloomToList((*bloomFiltersOfMonitors)[monitor],virusName,bf);
    }
  }
}

void searchVaccinationStatus(char citizenId[], int fdboard[][2]){
  for(int i=0;i<numMonitors;i++){
    char tempBuff[]="/searchVaccinationStatus";
    sendMessage(fdboard[i][WRITE],tempBuff); // send the command to every monitor
    sendMessage(fdboard[i][WRITE],citizenId);  // also send the id of the citizen
  }

  int ended[numMonitors]; // flag board that contains if the reading from every pipe if finished
  ...
  while(flag){
    ...
    for(int i=0;i<numMonitors;i++){
      if((pfds[i].revents&POLLIN)){ // if something is written in this pipe
          char *message=readMessage(fdboard[i][READ]); // read the message from this monitor
    ...
    flag=0;
    for(int i=0;i<numMonitors;i++){   //check if all pipes have ended reading
      if(ended[i]==0)
